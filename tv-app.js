...
function wakeTv(btn){
  ...
  return fetch("/power-on", { cache: "no-store" })
    .then(r => r.json())
    .then(r => {
      if (r.ok){
...
(function setupCommandsPage(){
  ...
  if (cf) cf.onclick = () => {
    ...
    fetch("/find-tv", { cache: "no-store" }).then(r => r.json())
