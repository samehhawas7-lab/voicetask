...
class WebOSRemote {
  ...
  _openPointer(raw){
    const url = this._pointerUrl(raw);
    if (url !== raw) diag("أُعيد توجيه قناة الأزرار إلى: " + url);
    try {
      const ws = new RelaySocket(url);
      ws.onopen  = () => { this.pointer = ws; diag("✓ قناة الأزرار جاهزة"); };
      ws.onclose = (ev) => {
        if (this.pointer === ws) this.pointer = null;
        diag("✗ أُغلقت قناة الأزرار (رمز " + (ev && ev.code) + ")");
      };
      ws.onerror = () => diag("✗ خطأ في قناة الأزرار");
    } catch (e){
      diag("✗ تعذّر فتح قناة الأزرار: " + e.message);
    }
  }
