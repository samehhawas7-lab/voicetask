...
(function setupProjector(){
  ...
  $("projFind").onclick = () => {
    ...
    fetch("/proj/find", { cache:"no-store" }).then(r => r.json()).then(r => {
  ...
  const press = (name, el) => {
    ...
    fetch("/proj/key?name=" + encodeURIComponent(name), { cache:"no-store" })
      .then(r => r.json())
  ...
  $("projWake").onclick  = () => { buzz(20); fetch("/proj/wake",  {cache:"no-store"}).then(()=>toast("أُرسلت إشارة التشغيل")); };
  $("projSleep").onclick = () => { buzz(20); fetch("/proj/sleep", {cache:"no-store"}).then(()=>toast("أُطفئت الشاشة")); };
  ...
  function loadApps(){
    ...
    fetch("/proj/apps", { cache:"no-store" }).then(r => r.json()).then(r => {
      ...
      r.apps.forEach(a => {
        ...
        btn.onclick = () => {
          buzz();
          fetch("/proj/app?pkg=" + encodeURIComponent(a.pkg), { cache:"no-store" })
            .then(r2 => r2.json())
