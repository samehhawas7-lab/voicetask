...
class WebOSRemote {
  ...
  async _afterReady(){
    ...
    try {
      ...
      if (res && res.socketPath){
        diag("عنوان قناة الأزرار من التلفزيون: " + res.socketPath);
        this._openPointer(res.socketPath);
      } else {
        diag("⚠ التلفزيون ما أعطى عنواناً لقناة الأزرار");
        this.onPointer(false, "التلفزيون ما أعطى عنواناً للقناة");
      }
    } catch (e){
      diag("⚠ فشل طلب قناة الأزرار: " + e.message);
      this.onPointer(false, "فشل طلب القناة: " + e.message);
    }
  ...
  async retryPointer(){
    ...
    const res = await this.request(SSAP.pointer);
    if (!res || !res.socketPath) throw new Error("التلفزيون ما أعطى عنواناً");
    diag("العنوان الجديد: " + res.socketPath);
    this._openPointer(res.socketPath);
  }
...
tv.onPointer = (ok, reason) => {
  ...
  bar.hidden = !!ok;
  if (!ok){
    $("padWarnText").textContent =
      "أزرار التنقل ولوحة اللمس معطّلة — " + (reason || "قناة الأزرار مغلقة");
  }
};
